namespace Aufgabe2neu {
    ...
    function game() {
        ...
        let Anzahl: number;
        let Eingabe: string = prompt("Gebe ein mit wie vielen Karten du spielen möchtest!");
        Anzahl = Number(Eingabe);

        for (let Zahl: number = 0; Zahl < Anzahl; Zahl++) {

            let randomKarte: number = ZufaelligeNummer(AllCards.length);
